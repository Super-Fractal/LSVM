...
def process_video_layers(
    ...
    text_exponent_for_this_layer,          # (str) Text to display on the first layer (layer 0)
    ...
    # このレイヤーは常に作成される
    try:
        text_exponent_for_this_layer = int(text_exponent_for_this_layer)
        status_messages.append(f"Creating First Layer ...")
        progress(0, desc="Creating First Layer") # Gradioプログレスバー更新
        layer0_output_path = os.path.join("output_temp", "layer0.mp4")
  
        TEXT_THRESHOLD_FOR_SCIENTIFIC_NOTATION = 20 # この回数を超えると指数表記
        print(text_exponent_for_this_layer)
        ...
        video_filter_layer0 = (
            ...
            f"text='{text_to_show_on_layer}':"        # 表示テキスト
...
with gr.Blocks(theme=gr.themes.Default()) as demo:
    ...
    with gr.Row(): # 横並びのセクション
        with gr.Column(scale=1): # 左側のカラム
            ...
            num_loops_slider = gr.Slider(label="Number of Compositing Layers (iterations)", minimum=1, maximum=1000, value=1, step=1) # minを0に (レイヤー0のみも可能に)
            ...
            output_width_number = gr.Number(label="Output Width (pixels)", value=1920, precision=0)
            output_height_number = gr.Number(label="Output Height (pixels)", value=1080, precision=0)

        with gr.Column(scale=1): # 右側のカラム
            gr.Markdown("### Text Overlay Settings")
            text_exponent_for_this_layer = gr.Slider(label="Text of the fourth power of the first layer. If the exponent is 0, the result is 1.)", minimum=0, maximum=1000, value=0, step=1)
            font_path_input_textbox = gr.Textbox(
                label="Font Name or Full Font File Path", 
                value="C\:/Windows/Fonts/arial.ttf", # Windowsの例。環境に合わせて変更が必要
                placeholder="e.g., Arial or /usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
            )
            text_size_input_slider = gr.Slider(label="Text Size", minimum=10, maximum=500, value=120, step=1) # maxを少し大きく
